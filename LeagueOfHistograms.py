...
def do_plots():
    ...
    if cb_w_b_teammate.get() == 1:
        LoHPlots.wr_by_teammate(filtered_parsed_match_data, 3)

    if cb_w_b_partysize.get() == 1:
        LoHPlots.wr_by_partysize(filtered_parsed_match_data, 3)
...
cb_w_b_teammate = tkinter.IntVar(value=0)
cb_w_b_partysize = tkinter.IntVar(value=0)
...
tkinter.Checkbutton(root, text="Winrate by Party Size", variable=cb_w_b_partysize).grid(row=12, column=c2, sticky="w")
